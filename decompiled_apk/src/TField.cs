...
public class TField
{
	...
	public static string OpenWebGLPrompt(string title, string defaultText, bool isPassword)
	{
		return null;
	}
	...
	public void setFocusWithKb(bool isFocus)
	{
		...
		_ = inputType;
		_ = INPUT_TYPE_NUMERIC;
		_ = inputType;
		_ = INPUT_TYPE_PASSWORD;
		if ((int)Application.platform == 11 || (int)Application.platform == 8)
		{
			if (inputType == 0)
			{
				kb = TouchScreenKeyboard.Open("", (TouchScreenKeyboardType)0);
			}
			else if (inputType == 2)
			{
				kb = TouchScreenKeyboard.Open("", (TouchScreenKeyboardType)1);
			}
			else
			{
				kb = TouchScreenKeyboard.Open("", (TouchScreenKeyboardType)4);
			}
		}
		if (kb != null)
		{
