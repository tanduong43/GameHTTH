public class PlayerListServer : PaintListScreen
{
	...
	public override void paintInfo(mGraphics g, InfoMemList mem, int xpaint, int ypaint, int i, int wsub)
	{
		...
		g.fillRect(xpaint + 32, ypaint - 1, num, 29);
		g.setColor(9403484);
		g.fillRect(xpaint + 1, ypaint, 26, 27);
		g.fillRect(xpaint + 32 + 1, ypaint, num - 2, 27);
		...
		if (typeClan(type))
		{
			...
			mFont.tahoma_7b_black.drawString(g, mem.name, xpaint + 22, ypaint + 2, 0);
			mFont.tahoma_7_black.drawString(g, mem.info, xpaint + 22, ypaint + 1 + GameCanvas.hText, 0);
		}
		else if (type == 15)
		{
	...
	{
		g.setColor(11936290);
		g.fillRect(xbegin, ybegin + idSelect * hItem, 28, 29);
		g.fillRect(xbegin + 32, ybegin + idSelect * hItem, wSub - 32 - 60, 29);
	}
