...
public class ObjectData
{
	...
	public static MainImage getImageAll(short id, string strId, MyHashTable hash, short typeImage)
	{
		...
		if (mainImage == null)
		{
			...
			if (typeImage == 1000)
			{
				Debug.Log((object)string.Format("[OBJECT_DATA getImageAll] Created new MainImage for monster id={0}, getFromRms={1}", id, (mainImage.img != null) ? "FOUND" : "null"));
			}
		}
		...
		if (mainImage.img == null)
		{
			...
			if (mainImage.timeImageNull >= 200 || mainImage.timeImageNull == 1)
			{
				if (typeImage == 1000)
				{
					Debug.Log((object)$"[OBJECT_DATA getImageAll] Requesting load_image id={id}, typeImage={typeImage}, timeNull={mainImage.timeImageNull}");
				}
	...
	public static mImage getFromRms(short id, int typeImage, MyHashTable myhash)
	{
		...
		if (setIdOK(id) && id >= 0)
		{
			...
			if (typeImage == 1000 && array != null)
			{
				Debug.Log((object)$"[OBJECT_DATA getFromRms] Loaded SUB_image{id + typeImage} from RMS: {array.Length} bytes");
			}
		}
		if (array == null)
		{
			if (id >= 0)
			{
				if (typeImage == 1000)
				{
					Debug.Log((object)$"[OBJECT_DATA getFromRms] RMS null for monster id={id}, calling load_image");
				}
		...
		try
		{
			mImage mImage3 = mImage.createImage(array, 0, array.Length);
			if (mImage3 == null || mImage3.image == null || (Object)(object)mImage3.image.texture == (Object)null)
			{
				if (id >= 0)
				{
					Debug.LogWarning((object)$"[OBJECT_DATA getFromRms] Failed to decode image from RMS for id={id}, re-requesting from server");
					GlobalService.gI().load_image(id, (short)typeImage);
				}
				return null;
			}
		...
		catch (Exception ex)
		{
			if (id >= 0)
			{
				Debug.LogError((object)$"[OBJECT_DATA getFromRms] Exception decoding RMS image id={id}: {ex.Message}");
				GlobalService.gI().load_image(id, (short)typeImage);
			}
			return null;
		}
