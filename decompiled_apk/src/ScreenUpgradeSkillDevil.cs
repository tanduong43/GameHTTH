public class ScreenUpgradeSkillDevil : ScreenUpgrade
{
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 0:
			...
			{
				Skill_Info skill_Info3 = (Skill_Info)vecSkill.elementAt(IdSelect);
				GlobalService.gI().Devil_Upgrade(9, skill_Info3.ID, 104, 0);
			}
			break;
		case 1:
		{
			...
			ScreenUpgrade.idSkillUpgrade = ScreenUpgrade.mItemUpgrade[0].ID;
			Skill_Info skill_Info = Skill_Info.getSkillFromID(ScreenUpgrade.idSkillUpgrade);
			if (skill_Info == null)
			{
				for (int i = 0; i < Player.vecListSkill.size(); i++)
				{
					Skill_Info skill_Info2 = (Skill_Info)Player.vecListSkill.elementAt(i);
					if (skill_Info2.ID == ScreenUpgrade.idSkillUpgrade)
					{
			...
			if (skill_Info != null)
			{
				ScreenUpgrade.startLvDevil = skill_Info.LvDevilSkill;
			}
	...
	public override void update()
	{
		...
		setStep();
		marName.update2();
		if (ScreenUpgrade.isContinueUpgradeSkillDevil && ScreenUpgrade.isClickUpgradeSkillDevilAuto)
		{
	...
	public override void updatePointer()
	{
		...
		{
			updatePointerCheckBoxUpgradeSkillDevil();
		}
		if (Step != 0)
		{
			return;
