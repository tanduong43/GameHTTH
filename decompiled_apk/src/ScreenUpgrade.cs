...
public class ScreenUpgrade : MainScreen
{
	...
	public string nameItemUpgradeSkinNotEnough = string.Empty;
	...
	public static bool isClickUpgradeSkillDevilAuto = false;

	public static bool isContinueUpgradeSkillDevil = false;

	public static long timeStartUpgradeSkillDevilAuto;

	public static sbyte choiceDevilBeriOrRuby = -1;

	public static int startLvDevil = -1;

	public static short idSkillUpgrade = -1;

	public int xCheckBoxSkillDevil;

	public int yCheckBoxSkillDevil;

	public int wCheckBoxSkillDevil;

	public int hCheckBoxSkillDevil;
	...
	public virtual void paintPosItem(mGraphics g)
	{
		for (int i = 0; i < posUp.Length; i++)
		{
			...
			{
				xCheckBoxSkillDevil = posUp[1][0] - 10;
				yCheckBoxSkillDevil = posUp[1][1] - 25;
				g.drawImage(AvMain.imgBorderCombo, xCheckBoxSkillDevil, yCheckBoxSkillDevil, 3);
				mFont tahoma_7b_black3 = mFont.tahoma_7b_black;
				string text3 = "Auto";
				int height3 = tahoma_7b_black3.getHeight();
				int width3 = tahoma_7b_black3.getWidth(text3);
				tahoma_7b_black3.drawString(g, text3, xCheckBoxSkillDevil + 10, yCheckBoxSkillDevil - 5, 0);
				wCheckBoxSkillDevil = width3 + 20;
				hCheckBoxSkillDevil = height3;
				if (isClickUpgradeSkillDevilAuto)
				{
					AvMain.fraCheck.drawFrame(2, xCheckBoxSkillDevil, yCheckBoxSkillDevil, 0, 3, g);
				}
			}
	...
	public override void update()
	{
		...
		if (isContinueUpgradeSkillDevil && isClickUpgradeSkillDevilAuto)
		{
			startUpgradeSkillDevilAuto();
		}
	}
	...
	public void updateKHAM()
	{
		if (Step == 1 || Step == 3)
		{
			...
			else
			{
				...
				if (typeRebuild == 8 || typeRebuild == 9 || typeRebuild == 13 || typeRebuild == 10 || typeRebuild == 11)
				{
					if (Step == 1)
					{
						mSound.playSound(28, mSound.volumeSound);
		...
		else
		{
			...
			tickStep = 0;
			tickStop = 0;
			if (typeRebuild == 8 || typeRebuild == 13 || typeRebuild == 10 || typeRebuild == 11)
			{
				if (!string.IsNullOrEmpty(showServer))
				{
					GameCanvas.Start_Normal_Only_CmdClose_DiaLog(showServer);
					Interface_Game.addInfoPlayerNormal(showServer, mFont.tahoma_7_yellow);
				}
				mItemUpgrade[0] = null;
				mItemUpgrade[1] = null;
			}
			else if (typeRebuild == 9)
			{
				...
				if (isClickUpgradeSkillDevilAuto)
				{
					checkContinueOrStopSkillDevilAuto();
					return;
				}
				GameCanvas.Start_Normal_Only_CmdClose_DiaLog(showServer);
				mItemUpgrade[0] = null;
	...
	public void updatePointerCheckBoxUpgradeItem()
	{
		...
		{
			isClickUpgradeItemToSucess = !isClickUpgradeItemToSucess;
	...
	public void updatePointerCheckBoxUpgradeSuperItem()
	{
		if (GameCanvas.isPoint(xCheckBoxItem - 10, yCheckBoxItem - 12, wCheckBoxItem, hCheckBoxItem + 10))
		{
			isClickUpgradeSuperItemToSucess = !isClickUpgradeSuperItemToSucess;
	...
	public void updatePointerCheckBoxUpgradeSkillDevil()
	{
		if (GameCanvas.isPoint(xCheckBoxSkillDevil - 10, yCheckBoxSkillDevil - 12, wCheckBoxSkillDevil, hCheckBoxSkillDevil + 10))
		{
			isClickUpgradeSkillDevilAuto = !isClickUpgradeSkillDevilAuto;
			if (!isClickUpgradeSkillDevilAuto)
			{
				stopUpgradeSkillDevilAuto();
			}
			GameCanvas.isPointerSelect = false;
		}
	}

	public void startUpgradeSkillDevilAuto()
	{
		long num = mSystem.currentTimeMillis();
		int num2 = (int)(800.0 / (1.0 + (double)GameCanvas.percent / 100.0));
		if (num - timeStartUpgradeSkillDevilAuto >= num2)
		{
			if (idSkillUpgrade != -1 && (choiceDevilBeriOrRuby == 0 || choiceDevilBeriOrRuby == 1))
			{
				GlobalService.gI().Devil_Upgrade(12, idSkillUpgrade, 104, 0);
				isContinueUpgradeSkillDevil = false;
			}
			else
			{
				stopUpgradeSkillDevilAuto();
	...
	public void checkContinueOrStopSkillDevilAuto()
	{
		...
		for (int i = 0; i < Player.vecListSkill.size(); i++)
		{
			...
			if (skill_Info2.ID == idSkillUpgrade)
			{
		...
		if (skill_Info != null)
		{
			if (startLvDevil != -1 && skill_Info.LvDevilSkill > startLvDevil)
			{
				stopUpgradeSkillDevilAuto();
				GameCanvas.Start_Normal_Only_CmdClose_DiaLog(showServer);
				mItemUpgrade[0] = null;
		...
		for (int j = 0; j < Player.vecInventory.size(); j++)
		{
			...
			if (mainItem != null && mainItem.typeObject == 7 && mainItem.ID == 9)
			{
		...
		{
			stopUpgradeSkillDevilAuto();
			GameCanvas.Start_Normal_Only_CmdClose_DiaLog(showServer + "\n" + T.notEnoughItemUpgrade);
			mItemUpgrade[0] = null;
			mItemUpgrade[1] = null;
		}
		else
		{
			isContinueUpgradeSkillDevil = true;
			timeStartUpgradeSkillDevilAuto = mSystem.currentTimeMillis();
		}
	}

	public static void stopUpgradeSkillDevilAuto()
	{
		isClickUpgradeSkillDevilAuto = false;
		isContinueUpgradeSkillDevil = false;
		choiceDevilBeriOrRuby = -1;
		startLvDevil = -1;
		idSkillUpgrade = -1;
	}
	...
	public void startUpgradeSuperItemAuto()
	{
		...
		if (nameMaterialLost.Equals(string.Empty))
		{
	...
	public string getNameMaterialLost(int[] arrCheck)
	{
		...
		for (int i = 0; i < arrCheck.Length; i++)
		{
			if (arrCheck[i] == 1)
			{
				...
				if (i == 1)
				{
					text = (text.Equals(string.Empty) ? T.tortoiseshell : (text + T.andText + T.tortoiseshell));
				}
				if (i == 2)
				{
					text = (text.Equals(string.Empty) ? T.superShield : (text + T.andText + T.superShield));
				}
			}
		}
		return text;
