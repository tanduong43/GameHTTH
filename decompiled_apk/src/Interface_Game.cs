public class Interface_Game
{
	...
	public void load_Image_Pointer()
	{
		...
		if (GameCanvas.isTouch)
		{
			...
			mPosOther[4][0] = 0;
			mPosOther[4][1] = MotherCanvas.h / 2 - 12;
			mPosOther[5][0] = 66;
			mPosOther[5][1] = 45;
			setPosTouch();
			setPosMenu_TaiTho();
	...
	private void paintChangTab(mGraphics g)
	{
		...
		{
			timeChangeTab = 0;
			Player.currentTab = ((Player.currentTab == 0) ? ((sbyte)1) : ((sbyte)0));
		}
	}
	...
	public static void paintInfoPlayer(mGraphics g, int x, int y, bool isborder, mFont fontLv)
	{
		...
		AvMain.paintRect(g, num2 + 94, num + 2, 16, 16, 0, 4);
		mFont.tahoma_7b_yellow.drawString(g, "A", num2 + 94 + 8, num + 4, 2);
	}
	...
	public void TouchMoveIngame()
	{
		isMove = true;
		updateShowTime();
		int num = (GameCanvas.isTaiTho ? 8 : 3) + 94;
		int num2 = 3 + GameScreen.h12plus + 2;
		if (GameCanvas.isPoint(num - 4, num2 - 4, 24, 24))
		{
	...
	public static void updateShowEvent()
	{
		if (eventcur != null)
		{
			...
			{
				GameCanvas.isPointerSelect = false;
				InfoMemList infoMemList = InfoMemList.getEvent(eventcur.name, eventcur.typeEvent);
				if (infoMemList != null)
				{
					GameCanvas.eventScr.memCur = infoMemList;
				}
				infoMemList.setAction();
				eventcur = null;
			}
		}
