...
public class MapBackGround
{
	...
	public void setBackGround(sbyte type, short h)
	{
		...
		if (lastMap != typeMap)
		{
			...
			case 81:
				...
				mSpeedImg[0] = 16;
				mImg = new mImage[1];
				mImg[0] = mImage.createImage("/bg/b810.png");
				if (mImg[0] != null)
				{
					mImg[0].setWH();
				}
			...
			case 87:
				...
				mSpeedImg[0] = 16;
				mImg = new mImage[1];
				mImg[0] = mImage.createImage("/bg/b870.png");
				if (mImg[0] != null)
				{
					mImg[0].setWH();
				}
	...
	{
		if (typeMap == 8 || typeMap == 51 || typeMap == 17 || typeMap == 23 || typeMap == 22 || typeMap == 81 || typeMap == 87)
		{
			return false;
		}
		return true;
	...
	public void paint(mGraphics g)
	{
		...
		case 43:
		{
			...
			num71 = MainScreen.cameraMain.xCam - MainScreen.cameraMain.xCam;
	...
	public void paintLast(mGraphics g)
	{
		...
		case 42:
		{
			for (int n = 0; n < PosObj.Length && PosObj[n] != null; n++)
			{
				if (12 + PosObj[n][0] > MainScreen.cameraMain.xCam - 17 && 12 + PosObj[n][0] < MainScreen.cameraMain.xCam + MotherCanvas.w + 17)
				{
					g.drawRegion(mImg[2], PosObj[n][2] * 17, PosObj[n][3] * 30, 17, 30, PosObj[n][5], 12 + PosObj[n][0], mHBegin[0] - 30 + 73 + 90 + PosObj[n][1], 33);
				}
			}
			break;
		}
		case 81:
		case 87:
			if (mImg != null && mImg.Length != 0 && mImg[0] != null)
			{
				for (int i = MainScreen.cameraMain.xCam / mSpeedImg[0] % mWImg[0]; i < MotherCanvas.w + mWImg[0]; i += mWImg[0])
				{
					g.drawImage(mImg[0], MainScreen.cameraMain.xCam + MotherCanvas.w - i, mHBegin[0] + yeff, 0);
	...
	public void update()
	{
		...
		if (typeMap == 15 || typeMap == 41 || typeMap == 60 || typeMap == 61 || ((typeMap == 20 || typeMap == 21 || typeMap == 22) && GameCanvas.isLowGraOrWP_PvP()) || typeMap == 28 || typeMap == 31 || typeMap == 42 || typeMap == 33 || typeMap == 43 || typeMap == 63 || typeMap == 81 || typeMap == 87)
		{
			yeff = 0;
		}
		else if (typeMap == 6)
