...
public class Potion : MainItem
{
	...
	public override void Use_Item()
	{
		...
		else if (Hp_Mp_Other == 40 && (ModMenu.isAutoUseItem || DelaySkill.getDelay(indexHotKey).isCoolDown()))
		{
		...
		{
			if (!ModMenu.isAutoUseItem && ((GameScreen.player.Action == 4 && (Hp_Mp_Other == 1 || Hp_Mp_Other == 2)) || !DelaySkill.getDelay(indexHotKey).isCoolDown()))
			{
				return;
			}
			if (!ModMenu.isAutoUseItem && Hp_Mp_Other == 1 && GameScreen.player.Hp == GameScreen.player.maxHp)
			{
				Interface_Game.addInfoPlayerNormal(T.HPFull, mFont.tahoma_7_white);
				return;
			}
			if (!ModMenu.isAutoUseItem && Hp_Mp_Other == 2 && GameScreen.player.Mp == GameScreen.player.maxMp)
			{
