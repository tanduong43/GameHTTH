...
public class TabInventory : MainTabShop
{
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 23:
		{
			...
			mVector5.addElement(cmdAllMaterial);
			mVector5.addElement(cmdAllDiamond);
			mVector5.addElement(cmdAutoMaterial);
		...
		case 24:
		{
			...
			mVector4.addElement(cmdRemoveItem);
		...
		case 31:
			...
			if (itemCur != null)
			{
				MainItem mainItem5 = (MainItem)GameScreen.player.hashEquip.get(string.Empty + itemCur.typeEquip);
				switch (subIndex)
				{
				case 0:
		...
		case 37:
			GameCanvas.end_Dialog();
			numPotionInput = ((!CRes.checkNumber(input.tfInput.getText())) ? (-1) : ((short)int.Parse(input.tfInput.getText())));
	...
	public override mVector getMenuActionItem()
	{
		...
		if (mainItem != null)
		{
			...
			{
				vecInfoSS = MainItem.getInfoSS(itemCur);
				if (ModMenu.IsUsableItem(itemCur))
				{
					if (mVector2 == null)
					{
						mVector2 = new mVector();
					}
					string caption = "Dùng nhanh";
					string text = ((itemCur.name != null) ? itemCur.name.ToLower() : "");
					string text2 = ((itemCur.nameUse != null) ? itemCur.nameUse.ToLower() : "");
					if (text2.Contains("đốt") || text2.Contains("dot") || text.Contains("pháo") || text.Contains("phao"))
					{
						caption = "Đốt nhanh";
					}
					else if (text2.Contains("mở") || text2.Contains("mo") || text.Contains("rương") || text.Contains("ruong") || text.Contains("túi") || text.Contains("tui"))
					{
						caption = "Mở nhanh";
					}
					else if (!string.IsNullOrEmpty(itemCur.nameUse) && itemCur.nameUse != "null")
					{
						caption = itemCur.nameUse + " nhanh";
					}
					MainItem targetItem = itemCur;
					mVector2.insertElementAt(new iCommand(caption, new ModMenu.ModAction(delegate
					{
						ModMenu.ShowInputUseItemQuantity(targetItem.name);
					})), 0);
				}
				else
				{
					string text3 = ((itemCur.name != null) ? itemCur.name.ToLower() : "");
					string text4 = ((itemCur.namepaint != null) ? itemCur.namepaint.ToLower() : "");
					if (text3.Contains("rương") || text3.Contains("ruong") || text4.Contains("rương") || text4.Contains("ruong"))
					{
						if (mVector2 == null)
						{
							mVector2 = new mVector();
						}
						MainItem chestItem = itemCur;
						mVector2.insertElementAt(new iCommand("Mở nhanh", new ModMenu.ModAction(delegate
						{
							ModMenu.ShowInputChestQuantity(chestItem.name);
						})), 0);
					}
				}
			}
	...
	{
		mSystem.outz("Use " + idIcon + " size Shop " + vecShop.size());
	...
	{
		Thread thread = new Thread(cShowQuickOpenPotion);
