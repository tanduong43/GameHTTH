...
public class LoginScreen : MainScreen
{
	...
	public void resetPos()
	{
		...
		if (GameCanvas.isTouch)
		{
			if (cmdMenu != null)
			{
				cmdMenu.setPos(GameCanvas.isTaiTho ? 30 : 15, MotherCanvas.h - 15, AvMain.fraIconMenu, string.Empty);
			}
			if (cmdHelp != null)
			{
				cmdHelp.setPos(GameCanvas.isTaiTho ? (MotherCanvas.w - 30) : (MotherCanvas.w - 15), MotherCanvas.h - 15, AvMain.fraIconHome, string.Empty);
			}
			if (cmdLogin != null)
			{
				cmdLogin.setPos(MotherCanvas.hw, yBeginPaint + hShowPaper, AvMain.fraBtLogin, string.Empty);
	...
	public override void commandPointer(int index, int subIndex)
	{
		...
		case 1:
		{
			...
			if (GameMidlet.isPC)
			{
				cmdOnLowGraphicPc.caption = ((GameCanvas.lv != 0) ? (T.on + T.lowGraphic) : (T.off + T.lowGraphic));
	...
	public override void paint(mGraphics g)
	{
		...
		mFont.tahoma_7_black.drawString(g, "Ver: 1.2.7", MotherCanvas.w - 2, 2 + GameScreen.h12plus, 1);
	...
	public void setData()
	{
		...
		GlobalService.gI().Send_Potion_Quick_Open(1);
