public class MsgAutoFire : MsgDialog
{
	...
	public override void updatePointer()
	{
		...
		if (GameCanvas.isPointerSelect)
		{
			...
			int num3 = (((value != null) ? ((value.Length - 1) / 3) : 0) + 1) * hItem;
			if (GameCanvas.isPoint(num2 + 10, num + hItem, wDia, num3) && value != null)
			{
				if (value != null)
				{
					for (int i = 0; i < value.Length; i++)
					{
						int num4 = num2 + xbeginBuff + (wItem + 8) * (i % 3);
						int y = num + hItem - wItem / 4 + i / 3 * hItem;
						if (GameCanvas.isPoint(num4 - 4, y, hItem + 8, hItem))
						{
							if (value[i][1] == 0)
							{
								value[i][1] = 1;
			...
			if (GameCanvas.isPoint(num2, num + hItem + num3, wDia, hItem))
			{
				if (valueRevice == 0)
				{
					valueRevice = 1;
