public class MotherCanvas
{
	...
	public void checkZoomLevel()
	{
		...
		if (GameMidlet.isPC)
		{
			mGraphics.zoomLevel = ((GameCanvas.lv == 0) ? 1 : 2);
		}
		else if (GameMidlet.DEVICE == 0)
		{
			mGraphics.zoomLevel = 1;
		}
		else if ((width >= 1280 && height >= 960) || (height >= 1280 && width >= 960))
		{
			mGraphics.zoomLevel = 4;
		}
		else if ((width >= 960 && height >= 720) || (height >= 960 && width >= 720))
		{
			mGraphics.zoomLevel = 3;
		}
		else if ((width > 600 && height > 400) || (height > 600 && width > 400))
		{
		...
		{
			GameCanvas.loginScr.resetPos();
		}
		if (GameCanvas.fristLoginScr != null)
		{
			GameCanvas.fristLoginScr.resetPos();
		}
	}
