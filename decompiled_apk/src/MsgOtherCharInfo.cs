public class MsgOtherCharInfo : MsgDialog
{
	...
	public override void updatekey()
	{
		...
		if (flag)
		{
			if (idSelect == 8)
			{
				if ((MainItem)obj.hashEquip.get(string.Empty + idSelect) == null)
				{
					if ((MainItem)obj.hashEquip.get(string.Empty + 9) == null)
					{
						idSelect = num;
						return;
					}
			...
			else if (idSelect == 9 && (MainItem)obj.hashEquip.get(string.Empty + idSelect) == null)
			{
				if ((MainItem)obj.hashEquip.get(string.Empty + 8) == null)
				{
					idSelect = num;
					return;
				}
	...
	public override void updatePointer()
	{
		...
		if (GameCanvas.isPointerSelect)
		{
			...
			for (int j = 0; j < TabEquip.maxEquip; j++)
			{
				if ((j == 8 || j == 9) && (MainItem)GameScreen.player.hashEquip.get(string.Empty + j) == null)
				{
