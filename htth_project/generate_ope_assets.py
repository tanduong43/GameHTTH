...
def draw_anime_hemisphere_room_dome(w=864, h=540):
    ...
    - Kích thước vút cao: 4x: 864x540 (1x: 216x135), chiều cao vòm 115px (tăng +37% chiều cao)
    ...
    - Các vệt ánh sáng bóng kính phản chiếu (anime sheen) và đường kinh - vĩ tuyến 3D
    """
    ...
    # 1. Khối màng không gian bán cầu khép kín (LỚP PHỦ MÀU TRẮNG XANH DƯƠNG)
    pts = []
    # Cung vòm bán cầu phía trên (180 đến 360 độ)
    for deg in range(180, 361, 3):
        rad = math.radians(deg)
        px = cx + int(math.cos(rad) * rx)
        py = cy_base + int(math.sin(rad) * h_dome)
        pts.append((px, py))
    # Vành ellipse chân đáy tiếp đất (0 đến 180 độ)
    ...
    sheen_pts = []
    for deg in range(205, 275, 4):
        rad = math.radians(deg)
        px = cx + int(math.cos(rad) * (rx * 0.94))
        py = cy_base + int(math.sin(rad) * (h_dome * 0.96))
        sheen_pts.append((px, py))
    d.line(sheen_pts, fill=(255, 255, 255, 75), width=12)
    d.line(sheen_pts, fill=(255, 255, 255, 130), width=5)

    # 3. Hào quang tỏa sáng ra ngoài vòm cầu (Xanh dương thanh khiết)
    for dr, a, w_line in [(18, 25, 8), (12, 45, 6), (6, 75, 4)]:
        dome_halo = []
        for deg in range(180, 361, 4):
            rad = math.radians(deg)
    ...
    rx_mid = int(rx * 0.88)
    ry_mid = int(ry * 0.88)
    d.arc([cx - rx_mid, y_mid - ry_mid, cx + rx_mid, y_mid + ry_mid], start=0, end=360, fill=(150, 225, 255, 120), width=3)
    d.arc([cx - rx_mid, y_mid - ry_mid, cx + rx_mid, y_mid + ry_mid], start=0, end=360, fill=(255, 255, 255, 140), width=1)

    for m_f in [-0.6, -0.25, 0.25, 0.6]:
        m_pts = []
        for deg in range(180, 361, 4):
            rad = math.radians(deg)
            px = cx + int(math.cos(rad) * (rx * (1.0 - abs(m_f)*0.6))) + int(m_f * rx * 0.5)
            py = cy_base + int(math.sin(rad) * h_dome)
            m_pts.append((px, py))
        d.line(m_pts, fill=(160, 230, 255, 95), width=2)
    ...
    d.ellipse([cx - rx, cy_base - ry, cx + rx, cy_base + ry], outline=(100, 210, 255, 190), width=4)
    d.ellipse([cx - rx + 4, cy_base - ry + 4, cx + rx - 4, cy_base + ry - 4], outline=(255, 255, 255, 225), width=2)

    # 7. Kim cương định vị không gian quanh vòm cầu
