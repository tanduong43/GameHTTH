...
def recolor_blast_natural(im_in):
    ...
    # Non-brown energy: yellow-white fire/spike/ring
    is_energy = m & ~is_brown & (lum >= 50)
    t = np.clip((lum[is_energy] - 50) / 205.0, 0, 1)
    new_r[is_energy] = np.clip(210 + t * 45, 0, 255)
    new_g[is_energy] = np.clip(130 + t * 125, 0, 255)
    new_b[is_energy] = np.clip(t * 70, 0, 70)
...
sequence = (
    ...
    [7, 7] +              # Chạm đất 80ms
    [8, 8, 8, 8] +        # Nện đất địa chấn đỏ tím 160ms
    [9, 9, 9, 9] +        # Cột lửa đỏ tím phun trào 160ms
    [10, 10, 10, 10, 10, 10] +  # 24312: Vỡ toác đất đá bùng nổ 240ms
    [11] * 48 +                 # 24312: Vết nứt vỡ toác đất giữ lại trên mặt đất ~2 giây (1920ms)
    [12, 12, 12, 12, 12, 12, 12, 12] # Khói bụi tản biến dần 320ms
)
