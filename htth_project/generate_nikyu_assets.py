...
# ==============================================================================
# 1. TẠO EFFECT 910: PAD HO (ÁP LỰC PHÁO - CHƯỞNG BAY NGANG KÈM VÒNG SÓNG ÂM)
# ==============================================================================
def draw_sonic_paw_cannon(draw, cx, cy, rx, ry, scale=4.0, has_tail=True, tail_len=24, is_charging=False):
    """Vẽ chưởng khí nén bàn tay gấu bay ngang với vệt tốc độ và cung xé gió."""
    if has_tail:
        tl = int(tail_len * scale)
        # Nón xé gió phản lực phía sau
        cone_pts = [
            (cx - int(rx * 0.6), cy),
            (cx - rx - tl, cy - int(ry * 1.1)),
            (cx - rx - int(tl * 0.5), cy),
            (cx - rx - tl, cy + int(ry * 1.1))
        ]
        draw.polygon(cone_pts, fill=(255, 150, 210, 45))

        # Các vệt sáng tốc độ song song
        for dy_ratio, w_ratio, col, lw in [
            (-0.7, 0.7, (255, 200, 230, 140), 1.5),
            (-0.4, 0.9, (255, 240, 245, 220), 2.0),
            (-0.15, 1.15, (255, 255, 255, 255), 2.5),
            (0.15, 1.15, (255, 255, 255, 255), 2.5),
            (0.4, 0.9, (255, 240, 245, 220), 2.0),
            (0.7, 0.7, (255, 200, 230, 140), 1.5),
        ]:
            y_pos = cy + int(dy_ratio * ry)
            x_end = cx - rx - int(tl * w_ratio)
            draw.line([(cx - int(rx * 0.5), y_pos), (x_end, y_pos)], fill=col, width=max(1, int(lw * scale)))

        # Vòng cung gợn sóng âm sau đuôi
        for arc_x, arc_h, alpha in [
            (cx - rx - int(tl * 0.3), int(ry * 0.8), 160),
            (cx - rx - int(tl * 0.65), int(ry * 0.6), 110)
        ]:
            draw.arc([arc_x - int(8 * scale), cy - arc_h, arc_x + int(4 * scale), cy + arc_h],
                     start=90, end=270, fill=(255, 255, 255, alpha), width=max(1, int(1.5 * scale)))

    # Hào quang khí nén phát sáng
    for r_add, a in [(int(7 * scale), 50), (int(4 * scale), 120), (int(2 * scale), 180)]:
        draw.ellipse([cx - rx - r_add, cy - ry - r_add,
                      cx + rx + r_add, cy + ry + r_add],
                     fill=(255, 90, 170, a), outline=(255, 235, 245, a + 40), width=max(1, int(scale)))

    # Viền đen tương phản
    draw.ellipse([cx - rx, cy - ry, cx + rx, cy + ry],
                 fill=(45, 35, 52, 255), outline=(255, 255, 255, 255), width=max(1, int(1.6 * scale)))

    # Lòng bàn tay đệm thịt chính
    prx = int(rx * 0.70)
    pry = int(ry * 0.65)
    draw.ellipse([cx - prx, cy - pry, cx + prx, cy + pry],
                 fill=(255, 105, 180, 255), outline=(255, 255, 255, 255), width=max(1, int(1.2 * scale)))
    # Ánh sáng bóng bên trong
    draw.ellipse([cx - int(prx * 0.5), cy - int(pry * 0.45),
                  cx + int(prx * 0.5), cy + int(pry * 0.2)],
                 fill=(255, 195, 225, 255))
    draw.ellipse([cx - int(prx * 0.25), cy - int(pry * 0.35),
                  cx + int(prx * 0.25), cy],
                 fill=(255, 255, 255, 255))

    # 4 Ngón tay gấu hướng lên trên chiều bay ngang
    toes = [
        (-0.68, -0.72, 0.24),
        (-0.24, -0.96, 0.27),
        ( 0.24, -0.96, 0.27),
    ...
    for bx, by, br in toes:
        ...
        draw.ellipse([tx - tr, ty - tr, tx + tr, ty + tr],
                     fill=(255, 105, 180, 255), outline=(255, 255, 255, 255), width=max(1, int(scale)))
        draw.ellipse([tx - tr * 0.4, ty - tr * 0.4, tx + tr * 0.4, ty + tr * 0.4],
                     fill=(255, 255, 255, 255))

    # Màng sóng xung kích xé gió phía trước mũi chưởng
    bow_pts = []
    for deg in range(-70, 71, 10):
        rad = math.radians(deg)
        bx = cx + math.cos(rad) * (rx + int(4 * scale))
        by = cy + math.sin(rad) * (ry + int(4 * scale))
        bow_pts.append((bx, by))
    if len(bow_pts) >= 2:
        draw.line(bow_pts, fill=(255, 255, 255, 240), width=max(1, int(2.0 * scale)))
...
def create_effect_910_pad_ho():
    ...
    # === ROW 1 (y = 0): 4 SPRITES BÀN TAY ĐỆM GẤU BAY NGANG TỪ TỤ KHÍ ĐẾN CỰC ĐẠI ===
    # Sprite 0: Mini Pad tụ khí tại tay (w=34, h=30) -> x=0, y=0
    draw_sonic_paw_cannon(draw, 17 * scale, 15 * scale, rx=10 * scale, ry=9 * scale, scale=scale, has_tail=False, is_charging=True)

    # Sprite 1: Pad bay vừa xé gió (w=54, h=38) -> x=36, y=0
    draw_sonic_paw_cannon(draw, (36 + 27) * scale, 19 * scale, rx=14 * scale, ry=12 * scale, scale=scale, has_tail=True, tail_len=18)

    # Sprite 2: Pad bay lớn cực mạnh (w=74, h=48) -> x=92, y=0
    draw_sonic_paw_cannon(draw, (92 + 37) * scale, 24 * scale, rx=21 * scale, ry=17 * scale, scale=scale, has_tail=True, tail_len=26)

    # Sprite 3: ĐẠI CHƯỞNG ÁP LỰC TỐI THƯỢNG (w=80, h=56) -> x=168, y=0
    draw_sonic_paw_cannon(draw, (168 + 40) * scale, 28 * scale, rx=26 * scale, ry=20 * scale, scale=scale, has_tail=True, tail_len=30)

    # === ROW 2 (y = 62): VÒNG SÓNG ÂM VÀ HIỆU ỨNG NỔ VA CHẠM TẦM XA ===
    # Sprite 4: Vòng sóng âm sonic ring đi sau (w=24, h=44) -> x=0, y=62
    cx4, cy4 = 12 * scale, (62 + 22) * scale
    for r_w, r_h, col, w in [(10 * scale, 20 * scale, (255, 200, 230, 150), 1.5),
                             (7 * scale, 15 * scale, (255, 245, 250, 220), 2),
                             (4 * scale, 10 * scale, (255, 255, 255, 255), 2)]:
        draw.ellipse([cx4 - r_w, cy4 - r_h, cx4 + r_w, cy4 + r_h], outline=col, width=max(1, int(w * scale)))

    # Sprite 5: Chớp nổ va chạm cực đại (Impact Starburst - w=64, h=50) -> x=26, y=62
    cx5, cy5 = (26 + 32) * scale, (62 + 25) * scale
    star_rays = [
        (0, -23), (6, -9), (28, -5), (10, 4), (22, 22), (4, 10),
        (-8, 23), (-10, 7), (-28, 9), (-12, -4), (-22, -19), (-5, -9)
    ]
    pts = [(cx5 + x * scale, cy5 + y * scale) for x, y in star_rays]
    draw.polygon(pts, fill=(255, 225, 90, 245), outline=(255, 255, 255, 255))
    draw.ellipse([cx5 - 13 * scale, cy5 - 10 * scale, cx5 + 13 * scale, cy5 + 10 * scale], fill=(255, 255, 255, 255))
    for ang in range(0, 360, 40):
        rad = math.radians(ang)
        x2 = cx5 + math.cos(rad) * 30 * scale
        y2 = cy5 + math.sin(rad) * 22 * scale
        draw.line([(cx5, cy5), (x2, y2)], fill=(255, 255, 255, 245), width=int(2 * scale))

    # Sprite 6: Sóng nổ bung tỏa (Expanding Detonation Wave - w=86, h=52) -> x=92, y=62
    cx6, cy6 = (92 + 43) * scale, (62 + 26) * scale
    for rx_w, ry_h, col, w in [(41 * scale, 24 * scale, (255, 190, 220, 130), 2),
                               (33 * scale, 19 * scale, (255, 235, 160, 180), 2),
                               (23 * scale, 13 * scale, (255, 255, 255, 255), 3)]:
        draw.ellipse([cx6 - rx_w, cy6 - ry_h, cx6 + rx_w, cy6 + ry_h], outline=col, width=int(w * scale / 2))

    # Sprite 7: Khói bụi tàn dư (Dust Puffs - w=38, h=34) -> x=180, y=62
    cx7, cy7 = (180 + 19) * scale, (62 + 17) * scale
    ...
    # --- Bảng khai báo sprites (id, x, y, w, h) ---
    small_imgs = [
        (0,   0,   0, 34, 30), # Sprite 0: Mini Pad (tụ khí tại lòng bàn tay)
        (1,  36,   0, 54, 38), # Sprite 1: Pad bay vừa xé gió
        (2,  92,   0, 74, 48), # Sprite 2: Pad bay lớn cực mạnh
        (3, 168,   0, 80, 56), # Sprite 3: ĐẠI CHƯỞNG Áp Lực Tối Thượng
        (4,   0,  62, 24, 44), # Sprite 4: Vòng sóng âm sonic ring
        (5,  26,  62, 64, 50), # Sprite 5: Chớp nổ va chạm cực đại (Impact Starburst)
        (6,  92,  62, 86, 52), # Sprite 6: Sóng nổ bung tỏa (Expanding Detonation Wave)
        (7, 180,  62, 38, 34), # Sprite 7: Khói bụi tàn dư (Dust Puffs)
    ]

    # HOẠT ẢNH: CHƯỞNG BAY NGANG TỪ TAY -> KÈM CÁC VÒNG SÓNG ÂM -> LAO TỚI MỤC TIÊU -> NỔ TUNG
    frames = [
        # Frame 0: Tụ khí tại lòng bàn tay nhân vật
        [(24 - 17, -15, 0, 0, 1)],
        # Frame 1: Phóng ra, xé gió bay ngang + Vòng sóng âm 1
        [
            (28 - 12, -22, 4, 0, 1),
            (65 - 27, -19, 1, 0, 1)
        ],
        # Frame 2: Tăng tốc cực mạnh + 2 Vòng sóng âm nối tiếp nhau
        [
            (45 - 12, -22, 4, 0, 1),
            (80 - 12, -22, 4, 0, 1),
            (115 - 37, -24, 2, 0, 1)
        ],
        # Frame 3: ĐẠI CHƯỞNG KHỔNG LỒ bay ngang tầm xa + 3 Vòng sóng âm dồn dập
        [
            (60 - 12, -22, 4, 0, 1),
            (95 - 12, -22, 4, 0, 1),
            (130 - 12, -22, 4, 0, 1),
        ...
        # Frame 4: Lao vào mục tiêu tầm xa + 2 Vòng sóng âm nối sau
        [
            (140 - 12, -22, 4, 0, 1),
            (175 - 12, -22, 4, 0, 1),
            (215 - 40, -28, 3, 0, 1)
        ],
        # Frame 5: ĐÂM TRÚNG MỤC TIÊU - NỔ TUNG VA CHẠM CỰC ĐẠI!
        [
            (185 - 12, -22, 4, 0, 1),
            (220 - 43, -26, 6, 0, 1),
            (220 - 32, -25, 5, 0, 1),
        ],
        # Frame 6: Sóng nổ bung tỏa cực đại + khói bụi va chạm
        [
            (222 - 43, -26, 6, 0, 1),
            (220 - 19, -17, 7, 0, 1),
        ],
        # Frame 7: Khói bụi tàn dư mờ dần
        [(222 - 19, -17, 7, 0, 1)],
        # Frame 8: Kết thúc
        []
    ]

    # SEQUENCE: 28 ticks (~0.93 giây)
    seq = [
        0, 0, 0,        # Tụ khí trước tay (3 ticks)
        1, 1, 1,        # Phóng ra nở vừa (3 ticks)
        2, 2, 2,        # Lao nhanh nở lớn (3 ticks)
        3, 3, 3,        # ĐẠI CHƯỞNG xé gió (3 ticks)
        4, 4, 4,        # Lao vào mục tiêu (3 ticks)
        5, 5, 5, 5,     # NỔ TUNG va chạm (4 ticks)
        6, 6, 6, 6,     # Sóng nổ bung tỏa (4 ticks)
        7, 7, 7, 7,     # Khói bụi tan dần (4 ticks)
        8               # Biến mất (1 tick)
    ]
...
# ==============================================================================
# 2. TẠO EFFECT 911: URSUS SHOCK (ĐẠI HÙNG CHƯỞNG - 3 BÀN TAY GẤU RƠI XUỐNG 1 LƯỢT 2 LẦN)
# ==============================================================================
def create_effect_911_ursus_shock():
    ...
    def draw_artistic_kuma_paw(draw, cx, cy, rx, ry, angle_deg=0, scale=4.0):
        ...
        # Dải vệt sáng xé gió lao nhanh từ trên cao xuống:
        trail_len = 18 * scale
        for off_r, a in [(-0.35, 100), (0, 220), (0.35, 100)]:
        ...
        # Bỏ hoàn toàn khối đen (cánh tay), giữ hào quang hồng và bàn tay gấu nghệ thuật nguyên bản:
    ...
    draw.ellipse([cx6 - 35*scale, cy6 - 13*scale, cx6 + 35*scale, cy6 + 13*scale],
    ...
    for pox, poy, pr in [(0, 0, 8), (-18, 1, 6), (18, 1, 6)]:
        pcx = cx6 + pox * scale
        pcy = cy6 + poy * scale
    ...
    # HOẠT ẢNH: 3 BÀN TAY GẤU CÙNG RƠI XUỐNG 1 LƯỢT 2 LẦN (ĐỢT 1 DẬM MỞ MÀN -> ĐỢT 2 DẬM CỰC ĐẠI URSUS SHOCK)
    frames = [
        # === ĐỢT 1: 3 BÀN TAY GẤU CÙNG RƠI DẬM MỞ MÀN ===
        # Frame 0: 3 Vòng dự báo đất + 3 Bàn tay gấu trên đỉnh mây trời (dy = -155..-175)
        [
            (-65, -10, 10, 0, 0), # Vòng dự báo trái
            (-18, -10, 10, 0, 0), # Vòng dự báo giữa
            ( 22, -10, 10, 0, 0), # Vòng dự báo phải
            (-65, -155, 1, 0, 1), # Bàn tay trái nghiêng
            (-27, -175, 0, 0, 1), # Đại bàn tay giữa
            ( 22, -155, 2, 0, 1), # Bàn tay phải nghiêng
        ],
        # Frame 1: 3 Bàn tay đồng loạt lao xé gió xuống lưng chừng (dy = -95..-110)
        [
            (-65, -10, 10, 0, 0),
            (-18, -10, 10, 0, 0),
            ( 22, -10, 10, 0, 0),
            ...
            (-27, -110, 0, 0, 1),
            ( 22,  -95, 2, 0, 1),
            (-18,  -25, 9, 0, 1), # Tia sét ma sát
        ],
        # Frame 2: 3 Bàn tay áp sát mặt đất (dy = -30..-40)
        [
            (-65, -30, 1, 0, 1),
            (-27, -40, 0, 0, 1),
            ( 22, -30, 2, 0, 1),
        ],
        # Frame 3: ĐÙNG! CẢ 3 BÀN TAY ĐỒNG LOẠT DẬM SẬP MẶT ĐẤT (FIRST IMPACT!)
        [
            (-62, -16, 3, 0, 1), # Dậm đất trái
            (-28, -16, 3, 0, 1), # Dậm đất giữa
            ( 18, -16, 3, 0, 1), # Dậm đất phải
            (-32, -22, 4, 0, 1), # Chớp nổ sáng
            (-45, -18, 5, 0, 0), # Sóng nén khí
            (-18, -25, 9, 0, 1), # Mảnh đá văng
        ],
        # Frame 4: Sóng nổ lan tỏa, khói bụi bốc lên 2 bên
        [
            (-52, -20, 7, 0, 0), # Khói bụi trái
            ( 14, -20, 8, 0, 0), # Khói bụi phải
            (-45, -18, 5, 0, 0), # Sóng nổ
            (-18, -35, 9, 0, 1), # Mảnh đá văng cao
        ],

        # === ĐỢT 2: 3 BÀN TAY GẤU CÙNG RƠI CỰC ĐẠI - URSUS SHOCK! ===
        # Frame 5: Đất rung tích tụ năng lượng + 3 Bàn tay gấu rực sáng xuất hiện trên đỉnh trời (Lần 2)
        [
            (-45, -12, 5, 0, 0),  # Vòng năng lượng nén cực đại dưới đất
            (-65, -10, 10, 0, 0), # Vòng dự báo trái
            ( 22, -10, 10, 0, 0), # Vòng dự báo phải
            (-65, -165, 1, 0, 1), # Bàn tay trái
            (-27, -185, 0, 0, 1), # Đại bàn tay giữa (CỰC ĐẠI)
            ( 22, -165, 2, 0, 1), # Bàn tay phải
        ],
        # Frame 6: 3 Bàn tay đồng loạt lao thiên thạch xé rách bầu trời xuống lưng chừng!
        [
            (-45, -12, 5, 0, 0),
            (-65, -105, 1, 0, 1),
            (-27, -120, 0, 0, 1),
        ...
        # Frame 7: Áp lực cực hạn - 3 Bàn tay áp sát mặt đất!
        [
            (-65, -35, 1, 0, 1),
            (-27, -45, 0, 0, 1),
            ( 22, -35, 2, 0, 1),
        ...
        # Frame 8: ĐÙNG ĐÙNG! CỰC ĐẠI IMPACT - CẢ 3 BÀN TAY DẬM SẬP MẶT ĐẤT!
        [
            (-62, -16, 3, 0, 1), # Dậm đất trái
            (-28, -16, 3, 0, 1), # Dậm đất giữa
            ( 18, -16, 3, 0, 1), # Dậm đất phải
            (-32, -22, 4, 0, 1), # Cực đại chớp nổ va chạm
            (-45, -18, 5, 0, 1), # Sóng xung kích
            (-18, -28, 9, 0, 1), # Mảnh đá văng tứ tung
        ],
        # Frame 9: ĐẠI SÓNG XUNG KÍCH URSUS SHOCK & DẤU ẤN 3 BÀN CHÂN IN LÚN NỨT ĐẤT
        [
            (-38, -12, 6, 0, 0), # Dấu ấn 3 bàn chân gấu in đất (onTop=0)
            (-45, -18, 5, 0, 1), # Đại sóng xung kích khổng lồ (onTop=1)
            (-52, -20, 7, 0, 0), # Khói bụi cuộn tung trái
            ( 14, -20, 8, 0, 0), # Khói bụi cuộn tung phải
            (-18, -40, 9, 0, 1), # Mảnh đá văng lên cao
        ],
        # Frame 10: SÓNG CHẤN QUÉT TOÀN MÀN HÌNH & KHÓI BỤI BỐC CAO
        [
            (-38, -12, 6, 0, 0), # Dấu ấn 3 bàn chân
            (-45, -18, 5, 0, 1), # Đại sóng xung kích
            (-56, -26, 7, 0, 0), # Khói bụi bốc cao trái
            ( 18, -26, 8, 0, 0), # Khói bụi bốc cao phải
        ],
        # Frame 11: Khói bụi cuộn cao và tàn dư chấn động mờ dần
        [
            (-38, -12, 6, 0, 0), # Dấu ấn mờ dần
            (-50, -24, 7, 0, 0), # Khói bụi mờ dần trái
            ( 12, -24, 8, 0, 0), # Khói bụi mờ dần phải
        ],
        # Frame 12: Biến mất hoàn toàn
        []
    ]

    # SEQUENCE: 44 TICKS (~1.45 GIÂY) - 3 BÀN TAY GẤU CÙNG RƠI 2 LẦN DẬM SẬP ĐẤT MÃN NHÃN
    seq = (
        # Đợt 1: 3 Bàn tay cùng rơi dậm đất mở màn (14 ticks)
        [0, 0, 0] +        # 3 Bàn tay xuất hiện đỉnh trời (3 ticks)
        [1, 1] +           # Lao xé gió xuống (2 ticks)
        [2, 2] +           # Áp sát mặt đất (2 ticks)
        [3, 3, 3, 3] +     # ĐÙNG! Cả 3 tay dậm đất đợt 1 (4 ticks)
        [4, 4, 4] +        # Sóng nổ & khói bụi (3 ticks)

        # Đợt 2: 3 Bàn tay cùng rơi cực đại Ursus Shock (30 ticks)
        [5, 5, 5, 5] +     # Đất rung & 3 tay giáng lâm đỉnh trời (4 ticks)
        [6, 6, 6] +        # Lao thiên thạch xé rách bầu trời (3 ticks)
        [7, 7] +           # Áp lực cực hạn sát mặt đất (2 ticks)
        [8, 8, 8, 8, 8] +  # ĐÙNG ĐÙNG! CẢ 3 TAY DẬM CỰC ĐẠI IMPACT (5 ticks)
        [9, 9, 9, 9, 9, 9] + # ĐẠI SÓNG XUNG KÍCH URSUS SHOCK & DẤU ẤN IN ĐẤT (6 ticks)
        [10, 10, 10, 10, 10] + # Sóng quét toàn màn hình & khói bụi cuộn cao (5 ticks)
        [11, 11, 11, 11] + # Khói bụi bốc cao tan dần (4 ticks)
        [12]               # Biến mất hoàn toàn (1 tick)
    )
...
def create_all_icons():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_icon_dir = os.path.normpath(os.path.join(script_dir, 'data', 'icon'))
