...
public class List_tich_tieu {
    ...
    public static void showTable(Player p) throws IOException {
        ...
        for (int i = 0; i < ENTRY.size(); i++) {
            ...
            m.writer().writeByte(p.tichTieuRubyCheck[i] == 1 ? 2 : (tongnap >= t.num ? 1 : 0));
  ...
  private static int getTongNap(Player p) {
    ...
    public static void process(Player p, Message m2) throws IOException {
        ...
        if (type == 1 && id < p.tichTieuRubyCheck.length && id < ENTRY.size() && p.tichTieuRubyCheck[id] != 1) {
            List<GiftBox> list = new ArrayList<>();
            List_tich_tieu listGet = ENTRY.get(id);
