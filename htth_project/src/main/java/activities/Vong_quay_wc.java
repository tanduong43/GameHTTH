    ...
    public static void process(Player p, Message m2) throws IOException {
        ...
        switch (action) {
            case 3: {
                ...
                m.writer().writeByte(Vong_quay_wc.ID_ITEM.length);
                for (int i = 0; i < Vong_quay_wc.ID_ITEM.length; i++) {
                    m.writer().writeByte(4);
                    m.writer().writeShort(ItemTemplate4.get_it_by_id(Vong_quay_wc.ID_ITEM[i]).icon);
                }
                p.conn.addmsg(m);
                m.cleanup();
                break;
            ...
            case 1: {
                int quant_reward = 0;
                if (action == 1) {
                    if (p.item.total_item_bag_by_id(4, 599) < 1) {
                        Service.send_box_ThongBao_OK(p, "Không đủ vé!");
                        return;
                    }
                    p.item.remove_item47(4, 599, 1);
                    quant_reward = 3;
                } else {
                    if (p.item.total_item_bag_by_id(4, 599) < 3) {
                        Service.send_box_ThongBao_OK(p, "Không đủ vé!");
                        return;
                    }
                    p.item.remove_item47(4, 599, 3);
                    quant_reward = 9;
                }
                ItemBag47[] list_reward = new ItemBag47[quant_reward];
                for (int i = 0; i < quant_reward; i++) {
                    list_reward[i] = Vong_quay_wc.get_random(p);
                }
