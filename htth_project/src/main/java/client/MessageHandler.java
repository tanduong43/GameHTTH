    ...
    public void login_into_char_select(short id) throws IOException {
        if (conn.list_char != null && id < conn.list_char.size()) {
            ...
            if (conn.p.map != null && (activities.BigBattle.isWaitingMap(conn.p.map)
                ...
                int targetMapId = 1;
                if (conn.p.originalMapId > 0 && !activities.BigBattle.isWaitingMapId(conn.p.originalMapId)) {
                    targetMapId = conn.p.originalMapId;
                } else if (conn.p.id_map_save > 0 && !activities.BigBattle.isWaitingMapId(conn.p.id_map_save)) {
                    targetMapId = conn.p.id_map_save;
                }
                System.out.println("[BigBattle Waiting Login safety]: player " + conn.p.name
                        + " logged in while in waiting map (" + conn.p.map.template.id + "), redirecting to village " + targetMapId);
                map.Map[] villageMap = map.Map.get_map_by_id(targetMapId);
                if (villageMap == null || villageMap.length == 0) {
                ...
                if (villageMap != null && villageMap.length > 0) {
                    conn.p.map = villageMap[0];
                    conn.p.x = (conn.p.originalX > 0 && targetMapId == conn.p.originalMapId) ? conn.p.originalX : 611;
                    conn.p.y = (conn.p.originalY > 0 && targetMapId == conn.p.originalMapId) ? conn.p.originalY : 250;
                }
                conn.p.type_pk = -1;
