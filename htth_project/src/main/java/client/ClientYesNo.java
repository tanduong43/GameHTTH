...
public class ClientYesNo {
    public static void process(Player p, Message m2) throws IOException {
        ...
        if (id == 1998) { // Xác nhận rời sảnh chờ Trận Chiến Lớn
            if (value == 0) { // Đồng ý
                activities.BigBattle.leaveWaitingRoom(p);
            }
            p.map_tele = null;
            return;
        }
        ...
        if (id == 991) {
            ...
            // chối)
            if (p.party != null) {
                activities.TowerChallenge.TowerChallengeLobby lobby = activities.TowerChallenge.ACTIVE_LOBBIES
                        .get(p.party);
                if (lobby != null) {
                    if (value == 0) {
                        lobby.memberAgreement.put(p.name, true);
                    } else {
                        lobby.memberAgreement.put(p.name, false);
                        // Notify leader immediately
                        Player leader = map.Map.get_player_by_name_allmap(lobby.leader.name);
                        if (leader != null) {
                            Service.send_box_ThongBao_OK(leader,
                                    "Thành viên " + p.name + " đã từ chối tham gia Vượt Liên Ải!");
                        }
                    }

                    // Update and send status board to leader and all voted members
                    for (Player mem : lobby.party.list) {
                        Boolean agreement = lobby.memberAgreement.get(mem.name);
                        if (agreement != null) { // Voted or is leader
                            Player onlineMem = map.Map.get_player_by_name_allmap(mem.name);
                            if (onlineMem != null) {
                                if (onlineMem.name.equals(lobby.leader.name)) {
                                    if (lobby.isAllAgreed()) {
                                        Service.send_box_yesno(onlineMem, 993, "Vượt Liên Ải - Bắt đầu",
                                                lobby.getStatusBoard()
                                                        + "\nTất cả thành viên đã đồng ý! Bấm 'Vượt ải' để bắt đầu.",
                                                new String[] { "Vượt ải", "Hủy" }, new byte[] { 0, -1 });
                                    } else {
                                        Service.send_box_yesno(onlineMem, 992, "Vượt Liên Ải - Chuẩn bị",
                                                lobby.getStatusBoard(),
                                                new String[] { "Cập nhật", "Hủy đăng ký" }, new byte[] { 0, -1 });
                                    }
                                } else {
                                    Service.send_box_ThongBao_OK(onlineMem, lobby.getStatusBoard());
                                }
                            }
                        }
                    }
        ...
        if (id == 992) {
            // Leader prepare lobby options
            if (p.party != null) {
                activities.TowerChallenge.TowerChallengeLobby lobby = activities.TowerChallenge.ACTIVE_LOBBIES
                        .get(p.party);
                if (lobby != null) {
                    if (value == 0) {
                        // Cập nhật (Refresh status)
                        if (lobby.isAllAgreed()) {
                            Service.send_box_yesno(p, 993, "Vượt Liên Ải - Bắt đầu",
                                    lobby.getStatusBoard() + "\nTất cả thành viên đã đồng ý! Bấm 'Vượt ải' để bắt đầu.",
                                    new String[] { "Vượt ải", "Hủy" }, new byte[] { 0, -1 });
                        } else {
                        ...
                        // Hủy đăng ký
                        activities.TowerChallenge.ACTIVE_LOBBIES.remove(p.party);
                        for (Player mem : p.party.list) {
                            Player onlineMem = map.Map.get_player_by_name_allmap(mem.name);
                            if (onlineMem != null) {
                                Service.send_box_ThongBao_OK(onlineMem, "Trưởng nhóm đã hủy đăng ký Vượt Liên Ải.");
                            }
                        }
                    }
                }
                ...
                if (lobby != null) {
                    if (value == 0) {
                        ...
                        for (int i = 0; i < lobby.party.list.size(); i++) {
                            ...
                            if (member == null || member.conn == null || !member.conn.connected || member.map == null
                                    || !member.map.equals(p.map) || member.get_key_boss() < 2
                                    || member.dungeon != null) {
                                ok = false;
                                break;
                            }
        ...
        if (value == 0) { // ok
            switch (id) {
                ...
                case 4800: {
                    ...
                    for (int i = 0; i < p.skill_point.size(); i++) {
                        if (p.skill_point.get(i).temp.indexSkillInServer == 900) {
                            Service.send_box_ThongBao_OK(p, "Bạn đã có haki quan sát rồi");
                            hasHaki = true;
                            break;
                        }
                    }
                    if (!hasHaki && p.item.total_item_bag_by_id(4, 800) > 0) {
                ...
                case 4801: {
                    boolean hasHaki = false;
                    for (int i = 0; i < p.skill_point.size(); i++) {
                        if (p.skill_point.get(i).temp.indexSkillInServer == 901) {
                            Service.send_box_ThongBao_OK(p, "Bạn đã có haki vũ trang rồi");
                            hasHaki = true;
                            break;
                        }
                    }
                    if (!hasHaki && p.item.total_item_bag_by_id(4, 801) > 0) {
                ...
                case 4802: {
                    boolean hasHaki = false;
                    for (int i = 0; i < p.skill_point.size(); i++) {
                        if (p.skill_point.get(i).temp.indexSkillInServer == 902) {
                            Service.send_box_ThongBao_OK(p, "Bạn đã có haki bá vương rồi");
                            hasHaki = true;
                            break;
                        }
                    }
                    if (!hasHaki && p.item.total_item_bag_by_id(4, 802) > 0) {
