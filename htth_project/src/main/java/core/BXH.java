...
public class BXH {
    ...
    public static List<InfoMemList> TOP_SAN_BOSS_LAN = new ArrayList<>();
    public static List<InfoMemList> TOP_NAU_BANH = new ArrayList<>();
    public static List<InfoMemList> TOP_NAP = new ArrayList<>();
    public static List<InfoMemList> TOP_AN_BANH = new ArrayList<>();
    ...
    public static void send(Player p, int type, int page) throws IOException {
        ...
        switch (type) {
            ...
            case 15: {
                updateTopSanBossLan();
                int bound1 = 0;
                int bound2 = BXH.TOP_SAN_BOSS_LAN.size();
                if (BXH.TOP_SAN_BOSS_LAN.size() > 10) {
                    if (((page + 1) * 10) > BXH.TOP_SAN_BOSS_LAN.size()) {
                        bound1 = 10 * page;
                        bound2 = BXH.TOP_SAN_BOSS_LAN.size();
                        while (bound1 >= bound2) {
                            bound1 -= 10;
                            page--;
                        }
                ...
                m.writer().writeUTF("Top Săn Boss Lân Sư Tử");
                m.writer().writeByte(page);
                m.writer().writeByte(bound2 - bound1);
                for (int i = bound1; i < bound2; i++) {
                    InfoMemList temp = BXH.TOP_SAN_BOSS_LAN.get(i);
                    InfoMemList.WriteInfoMemList(m.writer(), temp);
                }
                break;
            }
            case 16: {
                updateTopNauBanh();
                int bound1 = 0;
                int bound2 = BXH.TOP_NAU_BANH.size();
                if (BXH.TOP_NAU_BANH.size() > 10) {
                    if (((page + 1) * 10) > BXH.TOP_NAU_BANH.size()) {
                        bound1 = 10 * page;
                        bound2 = BXH.TOP_NAU_BANH.size();
                        while (bound1 >= bound2) {
                            bound1 -= 10;
                            page--;
                        }
                ...
                m.writer().writeUTF("Top Nấu Bánh Chưng Tân Niên");
                m.writer().writeByte(page);
                m.writer().writeByte(bound2 - bound1);
                for (int i = bound1; i < bound2; i++) {
                    InfoMemList temp = BXH.TOP_NAU_BANH.get(i);
                    InfoMemList.WriteInfoMemList(m.writer(), temp);
                }
                break;
            }
            case 17: {
                ...
                if (BXH.TOP_NAP.size() > 10) {
                    if (((page + 1) * 10) > BXH.TOP_NAP.size()) {
                        bound1 = 10 * page;
                        bound2 = BXH.TOP_NAP.size();
                        while (bound1 >= bound2) {
                            bound1 -= 10;
                            page--;
                        }
            ...
            case 18: {
                updateTopAnBanh();
                int bound1 = 0;
                int bound2 = BXH.TOP_AN_BANH.size();
                if (BXH.TOP_AN_BANH.size() > 10) {
                    if (((page + 1) * 10) > BXH.TOP_AN_BANH.size()) {
                        bound1 = 10 * page;
                        bound2 = BXH.TOP_AN_BANH.size();
                        while (bound1 >= bound2) {
                            bound1 -= 10;
                            page--;
                        }
                ...
                m.writer().writeUTF("Top Ăn Bánh");
                m.writer().writeByte(page);
                m.writer().writeByte(bound2 - bound1);
                for (int i = bound1; i < bound2; i++) {
                    InfoMemList temp = BXH.TOP_AN_BANH.get(i);
                    InfoMemList.WriteInfoMemList(m.writer(), temp);
                }
                break;
            }
    ...
    public static void updateTopDauTruong() {
        ...
        if (event.EventTet.getInstance().isDauTruongOpen()) {
            java.util.Map<String, Integer> currentScores = event.EventTet.getInstance().getCurrentArenaScores();
            if (currentScores != null && !currentScores.isEmpty()) {
                List<java.util.Map.Entry<String, Integer>> sorted = new ArrayList<>(currentScores.entrySet());
                sorted.removeIf(e -> e.getValue() == null || e.getValue() <= 0);
                sorted.sort((p1, p2) -> Integer.compare(p2.getValue(), p1.getValue()));

                for (java.util.Map.Entry<String, Integer> entry : sorted) {
                    String pName = entry.getKey();
                    if (pName == null || seenNames.contains(pName.toLowerCase())) continue;
                    seenNames.add(pName.toLowerCase());

                    InfoMemList temp = new InfoMemList();
                    Player p0 = Map.get_player_by_name_allmap(pName);
                    if (p0 != null) {
                        temp.id = p0.id;
                        temp.name = p0.name;
                        temp.level = (short) p0.level;
                        ...
                        loadOfflinePlayerAppearance(temp, pName);
                    }
                    temp.info = "Điểm: " + entry.getValue();
                    temp.rank = (short) list_add.size();
                    list_add.add(temp);
                    if (list_add.size() >= 50) break;
                }
            }
        } else {
            ...
            if (lastRecords != null && !lastRecords.isEmpty()) {
                for (int i = 0; i < lastRecords.size(); i++) {
                    ...
                    if (rec == null || rec.name == null || rec.kills <= 0 || seenNames.contains(rec.name.toLowerCase())) continue;
                    ...
                    if (p0 != null) {
                        ...
                        loadOfflinePlayerAppearance(temp, rec.name);
                    }
    ...
    public static void loadOfflinePlayerAppearance(InfoMemList temp, String pName) {
        temp.name = pName;
        temp.level = 1;
        temp.head = -1;
        temp.hair = -1;
        ...
        try {
            connection = SQL.gI().getCon();
            ps = connection.prepareStatement(
                    "SELECT `id`, `name`, `level`, `clazz`, `body`, `it_body`, `fashion`, `site` FROM `players` WHERE `name` = ? LIMIT 1;");
            ...
            if (rs.next()) {
                ...
                try {
                    Item_wear[] it = new Item_wear[8];
                    JSONArray js = (JSONArray) JSONValue.parse(rs.getString("it_body"));
                    for (int i1 = 0; i1 < js.size(); i1++) {
                        JSONArray js2 = (JSONArray) JSONValue.parse(js.get(i1).toString());
                        Item_wear temp2 = new Item_wear();
                        Item.readUpdateItem(js2.toString(), temp2);
                    ...
                    JSONArray jsSite = (JSONArray) JSONValue.parse(rs.getString("site"));
                    boolean is_show_hat = Byte.parseByte(jsSite.get(6).toString()) == 1;
                    if (!is_show_hat || it[1] == null) {
                        temp.hat = -1;
                    } else if (fashion_ != null && fashion_[1] != -1) {
                        temp.hat = fashion_[1];
    ...
    public static void updateTopSanBossLan() {
        List<InfoMemList> list_add = new ArrayList<>();
        Connection connection = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            while (rs.next()) {
                ...
                temp.info = String.format("Săn Boss: %s lần", Util.number_format(temp.thongthao));
                list_add.add(temp);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        if (list_add.size() > 0) {
            ...
            BXH.TOP_SAN_BOSS_LAN.clear();
            BXH.TOP_SAN_BOSS_LAN.addAll(list_add);
            list_add.clear();
        }
    }

    public static void updateTopNauBanh() {
        List<InfoMemList> list_add = new ArrayList<>();
        Connection connection = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            connection = SQL.gI().getCon();
            ps = connection.prepareStatement(
                    "SELECT `id`, `name`, `clazz`, `tet_banh_chung`, `body`, `it_body`, `fashion`, `site` FROM `players` WHERE `tet_banh_chung` > 0 ORDER BY `tet_banh_chung` DESC LIMIT 50;");
            rs = ps.executeQuery();
            while (rs.next()) {
                InfoMemList temp = new InfoMemList();
                temp.id = rs.getInt("id");
                ...
                temp.info = String.format("Nấu Bánh: %s lần", Util.number_format(temp.thongthao));
                list_add.add(temp);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        if (list_add.size() > 0) {
            ...
            BXH.TOP_NAU_BANH.clear();
            BXH.TOP_NAU_BANH.addAll(list_add);
            list_add.clear();
        }
    }

    public static void updateTopNap() {
        List<InfoMemList> list_add = new ArrayList<>();
        Connection connection = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            while (rs.next()) {
                ...
                if (pOnline != null) {
                    temp.id = pOnline.id;
                    temp.level = (short) pOnline.level;
                    temp.head = (short) pOnline.get_head();
                    temp.hair = (short) pOnline.get_hair();
                    temp.hat = pOnline.get_hat();
                } else {
                    PreparedStatement psPlayer = null;
                    ResultSet rsPlayer = null;
                    try {
                        psPlayer = connection.prepareStatement(
                                "SELECT `id`, `level`, `body`, `it_body`, `fashion`, `site` FROM `players` WHERE BINARY `name` = ? LIMIT 1;");
                        psPlayer.setString(1, charName);
                        rsPlayer = psPlayer.executeQuery();
                        if (rsPlayer.next()) {
                            temp.id = rsPlayer.getInt("id");
                            try {
                                JSONArray jsLevel = (JSONArray) JSONValue.parse(rsPlayer.getString("level"));
                                if (jsLevel != null && jsLevel.size() > 0) {
                                    temp.level = Short.parseShort(jsLevel.get(0).toString());
                                }
                            } catch (Exception ex) {}
                            ...
                            List<ItemFashionP> itfashionP = new ArrayList<>();
                            try {
                                JSONArray js0 = (JSONArray) JSONValue.parse(rsPlayer.getString("fashion"));
                                JSONArray js_temp_2 = (JSONArray) JSONValue.parse(js0.get(0).toString());
                                for (int i0 = 0; i0 < js_temp_2.size(); i0++) {
                                    JSONArray js_temp = (JSONArray) JSONValue.parse(js_temp_2.get(i0).toString());
                                    ItemFashionP tempf = new ItemFashionP();
                            ...
                            try {
                                JSONArray js = (JSONArray) JSONValue.parse(rsPlayer.getString("body"));
                                temp.head = (head_ != -1) ? head_ : Short.parseShort(js.get(0).toString());
                                temp.hair = (hair_ != -1) ? hair_ : Short.parseShort(js.get(1).toString());
                            } catch (Exception ex) {}
                            ...
                            try {
                                Item_wear[] it = new Item_wear[8];
                                JSONArray js = (JSONArray) JSONValue.parse(rsPlayer.getString("it_body"));
                                for (int i1 = 0; i1 < js.size(); i1++) {
                                    JSONArray js2 = (JSONArray) JSONValue.parse(js.get(i1).toString());
                                    Item_wear temp2 = new Item_wear();
                                    Item.readUpdateItem(js2.toString(), temp2);
                                ...
                                JSONArray jsSite = (JSONArray) JSONValue.parse(rsPlayer.getString("site"));
                                boolean is_show_hat = Byte.parseByte(jsSite.get(6).toString()) == 1;
                                if (!is_show_hat || it[1] == null) {
                                    temp.hat = -1;
                                } else if (fashion_ != null && fashion_[1] != -1) {
                        ...
                        if (rsPlayer != null) try { rsPlayer.close(); } catch (Exception e) {}
                        if (psPlayer != null) try { psPlayer.close(); } catch (Exception e) {}
                    }
                }
                list_add.add(temp);
            }
    ...
    public static void updateTopAnBanh() {
        List<InfoMemList> list_add = new ArrayList<>();
        java.util.Map<String, Integer> pointMap = event.EventTrungThu.getInstance().getPointMap();
        if (pointMap == null || pointMap.isEmpty()) {
            BXH.TOP_AN_BANH.clear();
            return;
        }

        List<java.util.Map.Entry<String, Integer>> sorted = new ArrayList<>(pointMap.entrySet());
        sorted.removeIf(e -> e.getValue() == null || e.getValue() <= 0);
        ...
        try {
            ...
            for (int i = 0; i < Math.min(50, sorted.size()); i++) {
                ...
                if (p0 != null) {
                    ...
                    temp.id = p0.id;
                    temp.name = p0.name;
                    temp.thongthao = points;
                    temp.level = (short) p0.level;
                    temp.head = (short) p0.get_head();
                    temp.hair = (short) p0.get_hair();
                    temp.hat = p0.get_hat();
                    temp.info = String.format("Ăn Bánh: %s Điểm", Util.number_format(temp.thongthao));
                    list_add.add(temp);
                } else {
                    ps.setString(1, pName);
                    rs = ps.executeQuery();
        ...
        if (list_add.size() > 0) {
            ...
            BXH.TOP_AN_BANH.clear();
            BXH.TOP_AN_BANH.addAll(list_add);
            list_add.clear();
        } else {
            BXH.TOP_AN_BANH.clear();
        }
