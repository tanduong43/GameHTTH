    ...
    public void process_msg(Message m) throws IOException {
        switch (m.cmd) {
             case 69:{
                ...
                case 87:{
                Qua_tu_hop1.process(conn.p, m);
                break;
            }
                        case -96:{
                List_tich_tieu.process(conn.p, m);
                break;
            }
            case 77:{
            ...
            case 82:{
                Vong_quay_wc.process(conn.p, m);
                break;
            }
            case 37:{
                Mission.process(conn.p, m);
                break;
            }
            case -90:{
            ...
            case 0: {
                if (conn.p != null) {
                    ...
                    if (conn.p.map.map_pvp != null) {// map pvp
                    ...
                    }else if (conn.p.map.map_LienTang != null
                            && conn.p.map.map_LienTang.type == 2) {
                        ...
                        Service.send_time_cool_down(conn.p, conn.p.map.map_LienTang.time_state,
                                "Tầng " + (conn.p.map.template.id - 202), 0);
                        if (conn.p.isdie) {
                            Service.use_potion(conn.p, 0, conn.p.body.get_hp_max(true));
                            Service.use_potion(conn.p, 1, conn.p.body.get_mp_max(true));
                            conn.p.isdie = false;
    ...
    public void login_into_char_select(short id) throws IOException {
        if (conn.list_char != null && id < conn.list_char.size()) {
            ...
            m2.writer().writeUTF(
                    ...
                    + "\n"
                    + "Chi Tiết : https://haitacngheo.com/ \n"
            );
            conn.p.list_msg_cache.add(m2);
            Message m3 = new Message(18);
            m3.writer().writeUTF("Open Đua Top Nhận Quà Khủng");
            m3.writer().writeUTF("\nThời gian: 9:00 ngày 2/6/2024.\n Theo dỏi tại zalo");
//                    + "Phần thưởng top level:\n"
...
//                    + "|5|TOP 10 nhân vật nạp nhiều nhất:\n"
