    ...
    public static void process(Player p, Message m2) throws IOException {
        ...
        if (value == 0) { // ok
            switch (id) {
                ...
                case 778: {
                    boolean suc = 30 > Util.random(120);
                    if (p.item.total_item_bag_by_id(4, 521) > 0 && p.item.total_item_bag_by_id(7, 16) > 0 && suc) {
                        p.item.remove_item47(4, 521, 1);
                        p.item.remove_item47(7, 16, 1);
                        p.item.update_Inventory(-1, false);
                        List<GiftBox> list_gift = new ArrayList<>();
                        ItemFashion itf = ItemFashion.get_item(93);
                        ...
                        Service.send_gift(p, 0, "Trang phục", "Quà đổi", list_gift, true);
                    }else if (p.item.total_item_bag_by_id(4, 521) > 0 && p.item.total_item_bag_by_id(7, 16) > 0 && !suc) {
                        p.item.remove_item47(4, 521, 1);
                        p.item.remove_item47(7, 16, 1);
                        p.item.update_Inventory(-1, false);   
                        Service.send_box_ThongBao_OK(p, "Thất bại");
                    } else {
                        Service.send_box_ThongBao_OK(p, "Bạn không đủ Mảnh TT Franky lịch lãm hoặc sao 8 cánh");
                    }
                    break;
                }
                case 64: {
                    if (p.check_id_danhhieu(p.id_danh_hieu_doi)) {
                        Service.send_box_ThongBao_OK(p, "Đã có danh hiệu!");
                        return;
                    }
                    if (DanhHieu.get_Id(p.id_danh_hieu_doi) != null) {
                        if (DanhHieu.get_Id(p.id_danh_hieu_doi).coint > p.conn.coin) {
                            Service.send_box_ThongBao_OK(p, "không đủ coin!");
                            return;
                        }
                        p.id_danh_hieu_da_so_huu.add(p.id_danh_hieu_doi);
                        p.conn.coin -= DanhHieu.get_Id(p.id_danh_hieu_doi).coint;
                        Service.send_box_ThongBao_OK(p, "Đổi thành công!");
                        p.id_danh_hieu_doi = -1;
                    } else {
                        Service.send_box_ThongBao_OK(p, "không tìm thấy danh hiệu!");
                    }
                    break;
                }
                case 63: {
                    if (p.clan.members.size() > 1) {
                        Service.send_box_ThongBao_OK(p, " còn một thành viên mới có thể giản tán!");
                        return;
                    }
                    for (Clan cl : Clan.ENTRY) {
                        if (cl == p.clan) {
                            Clan.ENTRY.remove(cl);
                            break;
                        }
                    }
                    SQL.gI().execute("delete from clan where `id` = " + p.clan.id + ";");
                    p.clan = null;
                    p.update_info_to_all();//thử tạo một clan xem giải tán dc chưa
                    Message msg = new Message(-52);
                    msg.writer().writeByte(10);
                    msg.writer().writeShort(p.id);
                    p.map.send_msg_all_p(msg, null, true);
                    msg.cleanup();
                    Service.send_box_ThongBao_OK(p, " Bạn đã giải tán thành công!");//dong thong bao
                    //chat
                    break;
                }
                case 62: {
                    if (p.map.map_LienTang == null) {
                        ...
                        for (int i = 0; i < p.party.list.size(); i++) {
                            ...
                            if (p0 != null && (!(p0.map.equals(p.map) && Math.abs(p0.x - p.x) < 200
                                    && Math.abs(p0.y - p.y) < 200) || p0.get_key_boss() < 2)) {
                                Service.send_box_ThongBao_OK(p, p0.name
                                        + " không đủ 2 chìa khóa phó bản hoặc không đứng gần");
                                p.data_yesno = null;
                                p.map_tele = null;
                                return;
                            }
                            if (p0.ltmax <= 0) {
                                Service.send_box_ThongBao_OK(p, p0.name
                                        + " Hôm nay đã tham gia 3/3 lần phó bản Liên Tầng");
                                p.data_yesno = null;
                                p.map_tele = null;
                                return;
                            }
                        ...
                        for (int i = 0; i < mapTemplate.list_mob.length; i++) {
                            ...
                            mob_add.x = temp.x;
                            mob_add.y = temp.y;
                            long hpNow = map_boss.map_LienTang.hpScale * 5 + map_boss.map_LienTang.hpScale;
                            hpNow = (hpNow * (100 + ((map_boss.template.id - 202) * 15))) / 100;
                            mob_add.hp_max = (int) hpNow;
                            mob_add.hp = mob_add.hp_max;
                            mob_add.level = map_boss.map_LienTang.levelMob;
                            mob_add.isdie = false;
                            mob_add.id_target = -1;
                        ...
                        map_boss.map_LienTang.time_state
                                = System.currentTimeMillis() + 60 * 2_000L;
                        //
                        listP.forEach(p0 -> {
                            try {
                                p0.map = map_boss;
                ...
                case 65: {
                    if (p.map.vuonCam == null) {
                        ...
                        for (int i = 0; i < p.party.list.size(); i++) {
                            Player p0 = Map.get_player_by_name_allmap(p.party.list.get(i).name);
                            if (!(p0 != null && p0.map.equals(p.map) && Math.abs(p0.x - p.x) < 100
                                    && Math.abs(p0.y - p.y) < 100) || p0.get_key_boss() < 2) {
                                Service.send_box_ThongBao_OK(p,
                                        p0.name + " không đủ 2 chìa khóa phó bản hoặc không đứng gần");
                                p.data_yesno = null;
                                p.map_tele = null;
                                return;
                            }
                        ...
                        for (int i = 0; i < p.party.list.size(); i++) {
                            ...
                            if (p0 != null && p0.map.equals(p.map)) {
                                ...
                                long dame = p0.body.get_dame(true);
                                dame = (dame * p0.body.get_dame_devil_percent()) / 100;
                                dameMax = Math.max(dameMax, dame);
                                lvMax = Math.max(lvMax, p0.level);
                            }
                        }
                        // create map
                        Map mapTemplate = Map.get_map_by_id(62)[0];
                        Map map_boss = new Map();
                        map_boss.template = mapTemplate.template;
                        map_boss.zone_id = (byte) 0;
                        map_boss.list_mob = new int[0];
                        map_boss.vuonCam = new VuonCam();
                        map_boss.vuonCam.hpMax = 1000;
                        map_boss.vuonCam.hp = map_boss.vuonCam.hpMax;
                        map_boss.vuonCam.hpMob = (int) (dameMax * 5 + dameMax);
                        map_boss.vuonCam.lvMob = lvMax;
                        map_boss.vuonCam.level = 1;
                        map_boss.vuonCam.state = 0;
                        map_boss.vuonCam.time_state = System.currentTimeMillis() + 11_000;
                        map_boss.vuonCam.mobs = new ArrayList<>();
                        map_boss.vuonCam.listRemoveMap = new HashMap<>();
                        map_boss.vuonCam.indexMob = -2;
                        //
                        listP.forEach(p0 -> {
                            try {
                                p0.map = map_boss;
                        ...
                        map_boss.vuonCam.add_name(p.party);
                    }
                    break;
                }
                case 61: {
                ...
                case 745:{
                    if (p.item.total_item_bag_by_id(4, id) > 0) {
                        Pet.addPetToPlayer(p, (short) (id - 699));
                ...
                case 16: {
                    if (p.map_boss_info != null) {
                        ...
                        for (int i = 0; i < p.map_boss_info.map.list_mob.length; i++) {
                            ...
                            if (mob_add.map.template.id == 202) {
                                mob_add.hp_max
                                        = (temp.mob_template.hp_max + Body.Point3_Template_hp[p.level]) * 15;
                                mob_add.hp = mob_add.hp_max;
                            }
                            p.map_boss_info.mob.add(mob_add);
                            // this.index_mob++;
//                            if (mob_add.map.template.id == 201) {
//                                mob_add.hp_max
//                                        = (temp.mob_template.hp_max + Body.Point3_Template_hp[p.level]) * 3;
//                                mob_add.hp = mob_add.hp_max;
//                            }
//                            p.map_boss_info.mob.add(mob_add);
                        }
