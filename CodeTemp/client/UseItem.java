...
public class UseItem {
    ...
    private static boolean use_item_4(Player p, int id) throws IOException {
        ...
        if (it_temp != null) {
            if (it_temp.type == 1 || it_temp.type == 2 && !p.map.is_qua_cau_poke(it_temp.id)) { // item hp mp
                    ...
                    case 414: { // ky nang don
                        ...
                        for (int i = 0; i < 3; i++) {
                            ...
                            if (skillSelect != null) {
                                ...
                                p.update_skill_exp(skillSelect.temp.ID, xpReq);
                            }
                        }
                        Service.send_box_ThongBao_OK(p, "Dùng kỹ năng đơn thành công");
                        break;
                    }
                    case 158: {
                        short id_add;
    ...
    public static synchronized void rename(Player p, String newChar) throws IOException {
        if (p.clan != null) {
            Service.send_box_ThongBao_OK(p, "Vui lòng rời băng trước khi đổi tên");
            return;
        }
        try (Connection connection = SQL.gI().getCon()) {
            if (newChar.length() < 5) {
                Service.send_box_ThongBao_OK(p, "Vui lòng nhập trên 5 ký tự.");
                return;
            }

            String newName = newChar;

            String sqlCheck = "SELECT COUNT(*) FROM account WHERE JSON_EXTRACT(`char`, '$[0]') = ?";
            try (PreparedStatement preparedStatement = connection.prepareStatement(sqlCheck)) {
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    if (resultSet.next() && resultSet.getInt(1) > 0) {
                        ...
                        return; // Hủy lệnh đổi tên nếu tên đã tồn tại
                    }
                }
            }
            String sqlAccounts = "UPDATE account SET `char` = JSON_REPLACE(`char`, '$[0]', ?) WHERE `char`->'$[0]' = ?";
            try (PreparedStatement preparedStatement = connection.prepareStatement(sqlAccounts)) {
                preparedStatement.setString(1, newName);
                preparedStatement.setString(2, p.name);
                preparedStatement.executeUpdate();
            }

            String sqlPlayers = "UPDATE player SET name = ? WHERE id = ?";
            try (PreparedStatement preparedStatement = connection.prepareStatement(sqlPlayers)) {
                preparedStatement.setString(1, newName);
                preparedStatement.setInt(2, p.id);
                preparedStatement.executeUpdate();
            }
