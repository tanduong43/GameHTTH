...
public class Service {
    ...
    public static void Send_UI_Shop(Player p, int type) throws IOException {
        ...
        switch (type) {
            ...
            case 21: {
                m.writer().writeUTF("Cửa Hàng Pet");//sao nữa ong
                m.writer().writeByte(4);
                short[] id_sell = ItemSell.get_it_sell_potion1(p);
                m.writer().writeShort(id_sell.length);
                for (int i = 0; i < id_sell.length; i++) {
                    m.writer().writeShort(id_sell[i]);
                    m.writer().writeShort(1);
            ...
            case 22: {
                m.writer().writeUTF("Cửa Hàng Noel");//sao nữa ong
                m.writer().writeByte(4);
                short[] id_sell = ItemSell.get_it_sell_potion2(p);
                m.writer().writeShort(id_sell.length);
                for (int i = 0; i < id_sell.length; i++) {
                    m.writer().writeShort(id_sell[i]);
                    m.writer().writeShort(1);
    ...
    public static void request_mob_in4(Player p, Message m2) throws IOException {
        ...
        if (temp == null && p.map.map_LienTang != null && p.map.map_LienTang.type == 2) {
        ...
        if (temp == null && p.map.vuonCam != null) {
            for (int i = 0; i < p.map.vuonCam.mobs.size(); i++) {
                Mob mob = p.map.vuonCam.mobs.get(i);
                if (!mob.isdie && mob.index == id) {
                    temp = mob;
                    break;
                }
        ...
        if (temp == null) {
            temp = Pokemon_normal.get_mob(p, id);
        }
        if (temp == null) {
            temp = Poke_huyen_thoai.get_mob(p, id);
        }
    ...
    public static void send_mob_info(Player p, Mob temp) throws IOException {
        ...
        } else if (temp.poke_nor_info != null) {
            m.writer().writeByte(temp.poke_nor_info.levelBoss); // lvthongthao
        } else if (temp.poke_huyen_thoai_info != null) {
            m.writer().writeByte(temp.poke_huyen_thoai_info.levelBoss); // lvthongthao
        } else {
                ...
                switch (cat) {
                    case 4: {
                        ...
                        if (template4 != null) {
                            ...
                            if (p.conn.coin < diemtichluy) {
                                Service.send_box_ThongBao_OK(p,
                                        "Không đủ " + temp_shop.point + " Coin");
                                return;
                            }
                            p.update_coin(-diemtichluy);
                            if (id == 221) {
